...
namespace BaseDAL
{
    public class AimService:IBase<Aim>
    {

        public int InsertRecord(Aim item)
        {
        ...
        public int UpdateRecord(Aim item)
        {
        ...
        public List<Aim> GetRecords(string where)
        {
        ...
        public Aim GetRecord(string where)
        {
            Aim aims = new Aim();
            using (SQLiteDataReader sdr = SQLiteHelper.ExecuteReader(CommandType.Text, "select * from source where " + where))
            {
        ...
        {
            string sql = "select b.billIndexID from bill b left join Source s on s.id=b.aim where b.aim>0 and (typeof(s.id)='null' or s.id='') group by b.billIndexID";
        ...
        {
            string sql = "select b.billIndexID from bill b left join Source s on s.id=b.aim where b.aim in (" + where + ") group by b.billIndexID";
            List<BillIndex> bi = new List<BillIndex>();
            using (SQLiteDataReader sdr = SQLiteHelper.ExecuteReader(CommandType.Text, sql))
            {
                if (sdr == null)
