...
namespace Main
{
    public partial class SourceInfo : Form
    {
        #region 单例模式
        private static SourceInfo _single;

        public static SourceInfo CreateForm()
        {
            if (_single == null)
                _single = new SourceInfo();
            else
                _single.Activate();
            return _single;
        }
        ...
        private void Init()
        {
            dgvBaseList = new SourceToControl().SourceToDgv(dgvBaseList, index);
        ...
        private void btnEdit_SaveClickHandleClick(object sender, EventArgs e)
        {
            ...
            if (EditOrInsert == 0)
            {
                ...
                new BaseManager<Source>().InsertBoxSize(bs);
            }
            //这里是修改记录方法
            if (EditOrInsert == 1)
            {
                ...
                foreach (DataGridViewRow item in dgvBaseList.Rows)    //循环更新
                {
                    ...
                    new BaseManager<Source>().UpdateBoxSize(bs);
                }
        ...
        private void btnEdit_DeleteClickHandleClick(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow item in dgvBaseList.SelectedRows)
            {
                ...
                new BaseManager<Source>().DeleteBoxSize(item.Cells["dgvBaseListID"].Value.ToString());
        ...
        {
            new SourceToControl().SourceCheck(dgvShipInfo, cbSelect.Text);
        }
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            new SourceToControl().SourceSearch(dgvShipInfo,cbSelect.Text, where.Substring(0, where.Length - 1));
        }
        /// <summary>
        /// 查询 校验或查询的结果列表
        /// </summary>
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (string item in context)
                {
                    ...
                    new BaseService<Source>().InsertRecord(p);
                }
                //重新装入到表格
                Init();
            }
        ...
        {
            new ExcelManager.ExcelHelper().SaveToBaseData(dgvBaseList, "目的港,来源,发往地,流向");
        }
        #endregion
    }
}
