...
namespace Main
{
    public partial class WorkPlaceInfo : Form
    {
        #region 单例模式
        private static WorkPlaceInfo _single;
        ...
        private void Init()
        {
            dgvBaseList = new WorkPlaceToControl().WorkPlaceToDgv(dgvBaseList, index);
        ...
        private void btnEdit_DeleteClickHandleClick(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow item in dgvBaseList.SelectedRows)
            {
                ...
                new BaseManager<WorkPlace>().DeleteBoxSize(item.Cells["dgvBaseListID"].Value.ToString());
            }
            lblError.Text = "";
            Init();
        }
        ...
        {
            new WorkPlaceToControl().WorkPlaceCheck(dgvShipInfo);
        }
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            new WorkPlaceToControl().WorkPlaceSearch(dgvShipInfo, where.Substring(0, where.Length - 1));
        }
        /// <summary>
        /// 查询 校验或查询的结果列表
        /// </summary>
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    WorkPlace p = new WorkPlace();
                    p.Name = item.Split(',')[1];
                    p.Code = item.Split(',')[0];
                    new BaseService<WorkPlace>().InsertRecord(p);
                }
