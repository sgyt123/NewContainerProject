...
namespace Main
{
    ...
    {
        #region 单例模式
        private static CompanyInfo _single;

        public static CompanyInfo CreateForm()
        {
            if (_single == null)
                _single = new CompanyInfo();
            else
                _single.Activate();
            return _single;
        }
        ...
        private void Init()
        {
            dgvBaseList = new CompanyToControl().CompanyToDgv(dgvBaseList, index);
        ...
        private void btnEdit_SaveClickHandleClick(object sender, EventArgs e)
        {
            ...
            if (EditOrInsert == 0)
            {
                ...
                new BaseManager<Company>().InsertBoxSize(bs);
            }
            //这里是修改记录方法
            if (EditOrInsert == 1)
            {
                ...
                foreach (DataGridViewRow item in dgvBaseList.Rows)    //循环更新
                {
                    ...
                    new BaseManager<Company>().UpdateBoxSize(bs);
                }
        ...
        private void btnEdit_DeleteClickHandleClick(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow item in dgvBaseList.SelectedRows)
            {
                ...
                new BaseManager<Company>().DeleteBoxSize(item.Cells["dgvBaseListID"].Value.ToString());
            }
            lblError.Text = "";
            Init();
        }
        ...
        {
            new CompanyToControl().CompanyCheck(dgvShipInfo);
        }
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            new CompanyToControl().CompanySearch(dgvShipInfo, where.Substring(0, where.Length - 1));
        }
        /// <summary>
        /// 查询 校验或查询的结果列表
        /// </summary>
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (string item in context)
                {
                    ...
                    p.Code = item.Split(',')[0];
                    p.Name = item.Split(',')[1];
                    new BaseService<Company>().InsertRecord(p);
                }
                //重新装入到表格
                Init();
            }
        ...
        {
            new ExcelManager.ExcelHelper().SaveToBaseData(dgvBaseList, "船公司");
        }
        #endregion
    }
}
